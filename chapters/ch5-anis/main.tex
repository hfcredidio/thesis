...
anisotropic~\cite{Englman1986}, like in stratified rocks formed by the layered
deposition of different types of sediment, each with have different physical
properties such as density and porosity. This anisotropy have important
influence
Aiming to model the structure of these
kind of media, Dayan \textit{et al.}~\cite{Dayan1991} introduced yet another
variant of the percolation model, called multi-layered percolation. 
...
where the line $p_1=p_2$ represent the regular isotropic percolation. The
larger the difference between $p_1$ and $p_2$ the more accentuated if the
anisotropy of the systems. To this effect it is common to reparametrize the
control parameter by defining
\begin{equation}
...
illustration of the multi-layered percolation process.

%This model is due to Dayan~\cite{Dayan1991}.
%Contributions by Parteli~\cite{Parteli2010}.

%Another universality class of strongly anisotropic phase transitions is yet
%another variation of the percolation model.

Just like isotropic percolation, the multi-layered version also undergoes a
second-order phase transition from a non-percolated to a percolated phase. In
this case we have two control parameters $p$ and $\Delta$, although the order
parameter (the relative size of the largest cluster) remains the same. In the
phase diagram of the square lattice, shown in Figure~\ref{fig:mlp_ps0}, we
observe a critical line separating the two phases. In the case $\Delta=0$ the
value of $p_c$ found is around $0.592$ which is the same as isotropic
percolation as expected, but it falls to $0.5$ as $\Delta\rightarrow 0.5$.
