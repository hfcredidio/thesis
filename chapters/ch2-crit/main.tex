...
despite its simplicity, it is capable to capture all of the main features of
ferromagnetic materials near the critical point. Today the Ising model forms
the basis of our understanding of phase transitions to the point that many
latter important models, like the $O(n)$ and Potts models~\cite{Wu1982}, can be
traced back to it.

The model consists in a set of classical spins $\{s_i\}$ that can take
one of two values $1$ or $-1$. They are arranged in a lattice and are allowed
to interact with their nearest neighbors. The Hamiltonian of the systems is
given by
\begin{equation}
    \mathcal{H}=
...
as many times on a positive magnetization as in a negative one, so the average
magnetization should be zero.

On the other hand, Peierls presented an argument for the presence of phase
transitions in $d>1$~\cite{Peierls1936}, showing that in infinite systems
(that is, in the thermodynamic limit) the thermal fluctuations at low
temperature are not enough to bring the system from a positive magnetization
state to a negative one (and vice-versa), so the system should be ``stuck''
...
The mistake in Ising's argument likes on the partition function,
\begin{equation}
    \mathcal{Z}=
...
the number of possible configurations $\{s_i\}$ is finite, and a finite sum of
exponentials is an analytical function. However, if the number of configurations is
infinite, which should happen in the thermodynamic limit, the analyticity
cannot be guaranteed.

Figure~\ref{fig:ising_phase2} shows the phase space of the 2D Ising model. We
observe a first-order phase transition when we flip the direction of the
external field $h$ for $T<T_c$. At $T=T_c$ the transition is continuous at
$h=0$. The value of the critical point is given by
\begin{equation}
    T_{c}=
    \frac{2J}{k_{b}\log\left(1+\sqrt{2}\right)}
    \approx2.27\frac{J}{k_{b}},
\end{equation}
and the magnetization for $T<T_c$~\cite{Yang1952}
\begin{equation}
    M={\left[1-\sinh^{-4}\left(\frac{2J}{k_{b}T}\right)\right]}^{1/8},
\end{equation}
from which we can determine that $\beta=1/8$. The values of the other
exponents of the Ising universality class can be seen in Table~\ref{tab:univ}.
...
probability $\exp(-\Delta E / T)$ (assuming normalized unities, where $J/k_b =
1$). Figure~\ref{fig:ising_phase}.
