...
The ``Loewner Evolution'' part of SLE was proposed in 1923 by Charles Loewner
in the context of Bierbach conjecture~\cite{Loewner1923} (proposed in 1916 and
eventually proved by de Branges in 1985). The starting point of the process
...
standard domain, or in math lingo $g:D\setminus\gamma\rightarrow D$. This is
called a \textit{uniformizing map}, and the crack  $\gamma$ goes by the more
formal name \textit{trace}. We distinguish two types of traces. If $r_2$ is an
interior point of $D$, the trace is said to be \textit{radial}, and if it
...
this point, we get a function $U_t:\mathbb{R}^+\rightarrow\partial D$ and
$g_t(\gamma_t)=U_t$. This is called the \textit{driving function} of the trace
in question. In his work, Loewner showed that the trace, the uniformizing on
map, and the driving function are connected by a differential equation.
...
continuity~\cite{Kager2004}. The choice capacity is intrinsically connected
...
mathematicians~\cite{Lawler2011}. Here, we'll go with the most common choice
$a(t)=2t$.
...
\begin{equation}
    ...
    g_0(z)=z.
\end{equation}
with $U_t\in\mathbb{R}$. Figure~\ref{fig:radchord} shows an example of 
...
simply connected, and the existence of $g_t$ cannot be guaranteed. This is
...
discontinuity, giving the trace a tree-like aspect.
